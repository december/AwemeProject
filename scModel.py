...
alpha = [1e-10, 1e-10, 1e-8, 1e-10, 1e-10, 1e-10, 1e-10]
total = 10000
threshold = 0.0001
...
def GradDes(cd, ncd, p, lr):
	...
	for item in ncd:
		...
		x = [(int(k) + 1) for k in x]
		for time in ncd[item]:
			grad[0] += DlnsDa(p[0], p[1], p[2], p[3:], x, time) * ncd[item][time] + 1.0 / p[0]
			grad[1] += DlnsDb(p[0], p[1], p[2], p[3:], x, time) * ncd[item][time] + 1.0 / p[1]
...
cnt = 0
p = [0.05, 0.05, 0.15, -0.01, -0.01, -0.01, -0.01] #a, b, theta, k1, k2, k3, k4

...
alpha = [1e-12, 1e-12, 1e-7, 1e-7, 1e-7, 1e-7]
total = 10000
threshold = 0.0001
...
def Gx(k, x):
	return (np.power(x[0], k[0]) + np.power(x[1], k[1])) * (np.power(x[2], k[0]) + np.power(x[3], k[2]))
...
def DlnhDk(a, b, theta, k, x, t):
	...
	for i in range(3):
		...
		if i == 2:
			...
			down = s * (np.power(x[2], k[0]) + np.power(x[3], k[2])) + b
		if i == 1:
			s *= (np.power(x[2], k[0]) + np.power(x[3], k[2]))
			up = s * np.power(x[i], k[i]) * np.log(x[i])
			down = s * (np.power(x[0], k[0]) + np.power(x[1], k[1])) + b
		if i == 0:
			up = s * np.power(x[2], k[0]) * np.log(x[2]) * (np.power(x[0], k[0]) + np.power(x[1], k[1])) + s * np.power(x[0], k[0]) * np.log(x[0]) * (np.power(x[3], k[2]) + np.power(x[2], k[0]))
			down = s * (np.power(x[1], k[1]) + np.power(x[0], k[0])) * (np.power(x[3], k[2]) + np.power(x[2], k[0])) + b
...
def DlnsDk(a, b, theta, k, x, t):
	...
	for i in range(3):
		...
		if i == 1:
			...
			result.append(-1 * s * np.power(x[i], k[i]) * np.log(x[i]))
		if i == 0:
			s *= -1 * ((np.power(x[2], k[0])) * np.log(x[2]) * (np.power(x[0], k[0]) + np.power(x[1], k[1])) + np.power(x[0], k[0]) * np.log(x[0]) * (np.power(x[2], k[0]) + np.power(x[3], k[2])))
			result.append(s)
	return result
...
cnt = 0
p = [0.0025, 0.0006, 0.728, 0, 0, 0] #a, b, theta, k1, k2, k3
